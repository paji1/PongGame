...
export class ChatController {
    ...
    async roomAddExistance(@Body() room: roomEntity, @Query("user") user: number)
    {
        ...
        if (room.type === permission.chat)
            return await this.service.rooms.create_chat(500, user)
    ...
    async humanMute(@Body() mute: MuteDto, @Query("type") type: string){
        //mute_user
        if (type === "umute")
            return await this.service.rooms.unmute_user(1, mute.targeted, mute.roomtarget);
    ...
    {
        return await this.service.rooms.give_room_admin(1,room, user)
    }
    /**
     * @description
     */
    ...
    {
        return await this.service.rooms.revoke_room_admin(1,room, user)
    }
}
