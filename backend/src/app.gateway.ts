...
export class AppGateway {
	...
	async handleDisconnect(client) {
		...
		if (!(await this.server.to(identifier).fetchSockets()).length) {
			const state = await this.prisma.user.update({
				where: { user42: identifier },
				data: { connection_state: current_state.OFFLINE },
			});
	...
	async sayHitoserver(
		...
		if ((await this.server.to(identifier).fetchSockets()).length == 1) {
			const state = await this.prisma.user.update({
				where: { user42: identifier },
				data: { connection_state: current_state.ONLINE },
			});
	...
	async notifyALL(user: string, status: string) {
		...
		friends.forEach(async (friend) => {
			...
			if ((await this.server.to(friend).fetchSockets()).length)
				this.server
					.to(friend)
					.emit("ACTION", { region: "ROOM", action: "status", data: { userh: user, status: status } });
		});
	...
	async informuser(nickname, invite, type) {
		if ((await this.server.to(nickname).fetchSockets()).length) this.server.to(nickname).emit(type, invite);
	...
	async getfriends(user: string) {
		...
		const list = friends.map((frien) => {
			if (frien.initiator_id.user42 == user) return frien.reciever_id.user42;
