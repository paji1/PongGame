...
const ButtonComponent = () => {
	return (
		...
		>
			<div
				className={`bg-background w-[70%] h-[70%] rounded-full shadow-xl
...
const GameFooter = () => (
	<div
		className={`border-solid border-textColor border-4 shadow-gameShadow h-22
	...
	>
		...
		<div className={`flex flex-col gap-3 p-4 box-border`}>
			<p className={`text-center capitalize text-xl`}>transcendence</p>
			<p className={`text-center capitalize`}>may the odds be ever in your favor</p>
		</div>
...
const GameBody = () => {
	...
	return (
		<div
			ref={gameBodyRef}
			className={`border-[.5rem] border-solid border-textColor
		sm:w-[576px] md:w-[691px] lg:w-[921px] xl:w-[1152px] 2xl:w-[1346px] w-[281px]
		sm:h-[324px] md:h-[389px] lg:h-[518px] xl:h-[648px] 2xl:h-[757px] h-[500px]`}
		>
			{preparation === EGamePreparationState.CONFIG_STATE ? (
				<GameSetup />
			) : preparation === EGamePreparationState.QUEUING_STATE ? (
				<QueueLoader />
			) : preparation === EGamePreparationState.READY_STATE ? (
				<PlayGround />
			) : null}
		</div>
	);
...
const GameFrame = () => {
	return <div className={`rounded-t rotate-90 sm:rotate-0 w-full h-full`}></div>;
};

const GameUI = () => (
	<div id="game-ui" className={`flex flex-col items-center h-auto inset-0`}>
		<div
			className={`flex flex-col items-center p-1 sm:p-5 mt-6
			`}
		>
			<GameBody />
			<GameFooter />
