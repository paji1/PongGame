...
const matchingHandler = (
	...
	if (matchingType === EMatchingType.INVITE && searchBar) queue.invite = searchBar.value;
...
const CheckButton = ({
	...
	<button
		className={`font-pixelify border-solid border-[.2rem] py-1 w-56 sm:w-64 rounded-full
...
const ConfigElems = () => {
	...
	useEffect(() => {
		if (matchingType === EMatchingType.INVITE) setSearchBar(searchForFriendRef.current);
		else setSearchBar(null);
	}, [matchingType]);
	...
	return (
		<div className={`flex flex-col lg:gap-8 md:gap-5 sm:gap-2 gap-8 border z-0 text-sm md:text-base lg:text-lg`}>
			<div className={`flex  flex-col gap-2 items-center justify-evenly`}>
				<CheckButton
					clickHandler={difficultyHandeler}
					text="easy"
					color="sucessColor"
					_key={EDifficulty.EASY}
					selected={difficulty}
				/>
				<CheckButton
					clickHandler={difficultyHandeler}
				...
				<CheckButton
					...
					text="hard"
					color="errorColor"
					_key={EDifficulty.HARD}
					selected={difficulty}
				/>
			</div>
			<div className={`flex flex-col gap-2 items-center justify-evenly`}>
				<CheckButton
					clickHandler={matchingTypeHandeler}
					text="Random"
					color="primary"
					_key={EMatchingType.RANDOM}
			...
			{matchingType === EMatchingType.INVITE ? (
				<input
					type="text"
					placeholder="Who..."
					ref={searchForFriendRef}
					className={`px-2 py-1 w-56 sm:w-64 border-solid border-textColor border-[.2rem] font-pixelify
				rounded-full focus:outline-none shadow-buttonShadow focus:border-textColor focus:text-textColor`}
					required
				/>
			) : null}
			<div className={`flex  flex-col gap-2 items-center justify-evenly`}>
				<button
					ref={startButtonRef}
					className={`py-1 border-solid border-textColor border-[.2rem] bg-buttonColor
...
const AnimatedElement = () => {
	...
	return <div className={`container overflow-hidden w-full h-full`} ref={container}></div>;
};
