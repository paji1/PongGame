...
export const CarouselItem = ({ hasit, item, image }: { hasit: boolean; item: achived; image: any }) => {
	return (
		<div
			className={` ${
				hasit ? "" : "grayscale"
			} snap-center m-auto carousel-item inline-flex flex-col place-items-center justify-around h-[345px]`}
		>
			<img
				className="carousel-img min-[0px]:w-[160px] min-[0px]:h-[190px] md:w-[280px] md:h-[340px] p-6 md:p-16 drop-shadow-[25px_30px_3px_rgba(0,0,0,0.40)]"
				src={image}
			/>
			<h1 className="mb-12 w-full carousel-item selection:bg-buttonColor min-[0px]:text-xl xl:text-2xl font-Nova font-bold hover:duration-700 hover:ease-in-out delay-[510ms]">
				<Tooltip description={item.description}>{item.name}</Tooltip>
			</h1>
		</div>
	);
};
