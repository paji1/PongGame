...
import { rooms, room } from "../types/room";
import Profile from "../assets/profile.png"
import ChatRoomAvatar from "../assets/ChatRoomAvatar.png"



 const GroupItem = ({selector, room , glimpse}: {selector : any, room : room, glimpse: string}) =>
{
        ...
        <div onClick={() => selector(room.id)} className="flex flex-col flex-auto cursor-pointer gap-2 " >
 ...
 const FriendItem = ({selector, room , glimpse}: {selector:any , room : room, glimpse: string}) =>
{
        glimpse = (glimpse.length > 25 ) ? (glimpse.substring(0, 25) + "...") : glimpse;
        const name = (room.rooms_members[0].user_id.nickname.length > 15) ? (room.rooms_members[0].user_id.nickname.substring(0, 15)  ) : room.rooms_members[0].user_id.nickname
 ...
 const  ChatList = ({rooms, query, roomselector}: {rooms:rooms[] | null,  query:string, roomselector:any}) =>
{
        var list;
        if (rooms)
        {
                if (query.length)
                {
                        ...
                        list = rooms.map((ob: rooms, index: number)=> 
                        {
                                let group = ob.rooms.name.toLowerCase();
                                let name  = ob.rooms.rooms_members[0].user_id.nickname.toLowerCase()

                                if (ob.rooms.roomtypeof === "chat" && name.includes(query.toLowerCase()))
                                        return (<FriendItem selector={roomselector} room={ob.rooms} glimpse="dsfsdfdsfds"/>)
                                else if (ob.rooms.roomtypeof !== "chat" && group.includes(query.toLowerCase()))
                                {
                                        return (<GroupItem selector={roomselector} room={ob.rooms} glimpse="dsfsdfdsfds"/>)
                                }
                        })
                } else {
                        ...
                        list = rooms.map((ob: rooms, index: number)=> 
                        {
                                ...
                                if (ob.rooms.roomtypeof === "chat" )
                                        return (<FriendItem selector={roomselector} room={ob.rooms} glimpse="dsfsdfdsfds"/>)
                                return (<GroupItem selector={roomselector} room={ob.rooms} glimpse="dsfsdfdsfds"/>)
                        })
                }
        }
        return (<div className="border-solid border-white border-2 flex flex-col flex-auto gap-2 overflow-y-scroll">
                {list}
        </div>)
}
