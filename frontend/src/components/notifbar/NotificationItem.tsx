...
{
	await fetch(`http://${ip}3001/invite/friend/invite?friend=${id}` , {credentials: 'include', method: "POST"}).catch(err => toast.error("ACCEPT: Network error"))
}

const RejectFriend = async (id:number) =>
{
	await fetch(`http://${ip}3001/invite/friend/invite?friend=${id}` , { credentials: "include", method: "DELETE"}).catch(err => toast.error("REJECT: Network error"))
}
