...
const AcceptFriend = async (id: number) => {
	const res = await fetch(`http://${ip}3001/invite/friend/invite?id=${id}`, {
		credentials: "include",
		method: "POST",
	});
	console.log(res);
			...
			<div className={`flex justify-between flex-auto flex-col sm:flex-row gap-2 `}>
				<div className="flex flex-col gap-1 justify-center">
					<h1 className={`text-lg text-primary `}>
						{" "}
						{!(notif.status === NotificationStatus.pending) ? notif.status : notif.issuer_id.nickname}
					</h1>
					<p className="text-sm">
				...
				<div
					className={`grid grid-cols-2 gap-3 content-evenly p-2
					 `}
				>
					{notif.status === NotificationStatus.pending && user?.id != notif.issuer_id.id ? (
						<>
							{" "}
							<button
								className="flex items-center justify-center col-1"
								onClick={() => routeinvites("ok", notif, socket)}
							>
								<svg
							...
							<button
								className="flex items-center justify-center col-1"
								onClick={() => routeinvites("no", notif, socket)}
							>
								<svg
					...
					<div>{user?.id == notif.issuer_id.id ? <>outgoing</> : <>incoming</>}</div>
				</div>
			</div>
		</div>
	);
